...
export default function Main(props) {
  ...
  useEffect(() => {
    if (props.loincId) {
        ...
        axios
          .get(`http://localhost:8080/paths/LOINC/${props.loincId}`)
  ...
  useEffect(() => {
    if (path.data && path.data[0]) {
      ...
      if (props.loincId.substring(0,2) === 'LP') {
        setCodeType('LP');
        axios
          .get(`http://localhost:8080/LP/LOINC/${props.loincId}`)
          .then(response => setLP(response));
        axios
          .get(`http://localhost:8080/LPLINK/LOINC/${props.loincId}`)
          .then(response => setLPLink(response));
        axios
          .get(`http://localhost:8080/LPMAP/LOINC/${props.loincId}`)
          .then(response => setLPMap(response));
      } else {
        setCodeType('');
        axios
          .get(`http://localhost:8080/entity/LOINC/${props.loincId}`)
          .then(response => setEntity(response));
        axios
          .get(`http://localhost:8080/LPLINK/LOINC/${props.loincId}`)
          .then(response => setLPLink(response));
        axios
          .get(`http://localhost:8080/LA/LOINC/${props.loincId}`)
          .then(response => setLA(response));
        axios
          .get(`http://localhost:8080/LGTERM/LOINC/${props.loincId}`)
          .then(response => setLGTerm(response));
        axios
          .get(`http://localhost:8080/panel/LOINC/${props.loincId}`)
          .then(response => setPanel(response));
        axios
          .get(`http://localhost:8080/LV/LOINC/${props.loincId}`)
