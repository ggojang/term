  ...
  */
  ...
  useEffect(() => {
    if (q.length > 1) {
      ...
      axios
        .get(`http://localhost:8080/search/SNOMEDCT?match=${matchType}&state=${state}&q=${q}&page=${page}&size=${size}`)
  ...
  useEffect(() => {
    if (q.length > 1 && page > 1) {
      ...
      */
      axios
        .get(`http://localhost:8080/search/SNOMEDCT?match=${matchType}&state=${state}&q=${q}${tmp}&page=${page}&size=${size}`)
        .then(response => setSearchResult(response));
    }
  }, [page]);
  ...
  useEffect(() => {
    if (q.length>1 && listCheckBox.length !==0) {
      ...
      */
      axios
        .get(`http://localhost:8080/search/SNOMEDCT?match=${matchType}&state=${state}&q=${q}${tmp}&page=1&size=${size}`)
